...
* TCP/UDP를 쓴다면, 클라이언트 1번이 채팅방에 글을 썼을 때 통신 방식은 다음과 같다. 클라이언트 1번과 서버가 1대1로 연결되고, 다른 클라이언트와 서버도 다시 각각 1대1로 연결되는 형식이며, 서버는 1번으로부터 받은 정보를 각 클라이언트들에게 독립적으로 전달한다.
...
* Publisher가 Subscriber들에게 한 번에 정보를 `1:n` 형식이다.
...
* 통신 회사에서 1:m으로 보낼 때 자주 사용한다. 보내는 곳은 하나지만 받을 곳은 매우 많다.
* 클라우드 컴퓨팅 위에서 동작한다.
