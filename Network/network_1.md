...
* 네트워크 계층에서는 packet을 받아서 내보낸다. 고로 그것을 하기 위한 정보 필요하고, 그래서 네트워크계층에서는 **table이 항상 있다**.
...
* 네트워크 계층에서는 Table 필수 -  패킷 안의 데이터그램 안의 destination address 보고 출력하는 interface로 보냄
...
## Network Performance
* OS 7계층 전체에 사용 가능한 단어와 의미

### Delay
* Packet 전달 시 Source host -> Destination host 까지의 걸리는 시간
* 4가지 요소 개입
	* transmission delay : 인간이 만든 것, 표준 규격에 의거 전달하는 전송 속도 있는 것.
	* propagation delay : 자연이 만들어내는 속도 (재료에 따라…)
	* processing delay : 하드웨어와 소프트웨어를 사용하기 때문에 발생하는 지연
	* queuing delay : 대기로 인해 발생하는 지연
* 계산 가능, 계산 후 우리의 요구 수준 부합하는지 판단 필요.

### Throughput
* 첫번째 의미 : 입력이 얼마나 흐를 수 있는지? (속도, )
...
## Addressing
* IPv4 address 
* ip address는 **줄**에 주는 것: 컴퓨터가 두개 이상의 줄을 갖고 있으면 줄마다 address 부여.
* 라우터가 address 갖는 것이 아니고, 줄에 address 부여.

### IPv4 Address Space
* 패킷이 있다한다면 주소 적는 field 있고, 이러한 address space 유한한 크기로  존재.
...
* but 이렇게 잘라주기에는 컴퓨터쓰는 사람 너무 많고 문제가 있음
